...
const PADDLE_REDUCTION_FACTOR = 0.8;
...
const Game: React.FC = () => {
  ...
  const calculateNewVelocity = (
    ...
    const normalizedRelativeIntersectionY =
      relativeIntersectY / (PADDLE_HEIGHT / 2);
  ...
  const showBounceMessage = useCallback(
    (isLeft: boolean) => {
  ...
  useEffect(() => {
    ...
    const updateGameState = (time: number) => {
      ...
      if (
        ...
        setLeftPaddleY((prev) =>
          Math.min(prev + PADDLE_SPEED, GAME_HEIGHT - PADDLE_HEIGHT),
        );
      }
      ...
      if (gameStarted) {
        ...
        setBallPosition((prev) => {
          ...
          if (newX < 0 && time - lastScoreTimeRef.current > 1000) {
            ...
            newX + BALL_SIZE > GAME_WIDTH &&
            time - lastScoreTimeRef.current > 1000
          ) {
            setLeftScore((s) => s + 1);
            setRightLives((lives) => {
              const newLives = lives - 1;
      ...
      <div className={styles.gameContainer}>
        ...
        <div className={styles.gameWindow}>
          ...
          {!gameStarted && !gameOver && (
            <div className={styles.startMessage}>Press Space to Start</div>
          )}
          {gameOver && (
        ...
        <div className={styles.gameStats}>
          Level: {level} |{" "}
          <span className={styles.gameTimer}>Time: {Math.ceil(countdown)}</span>{" "}
          | Speed: {ballSpeed.toFixed(2)} | Paddle: {paddleHeight.toFixed(0)}
        </div>
